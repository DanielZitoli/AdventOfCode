...
long findNumberOfPaths(map<string, int> &namesToId, vector<vector<int>> &adj, vector<int> &order, string source, string destination) {
  int sourceId = namesToId[source], destId = namesToId[destination];
  int n = adj.size();
...
int main() {
  ...
  long StoF = findNumberOfPaths(namesToId, adj, order, "svr", "fft");
  long StoD = findNumberOfPaths(namesToId, adj, order, "svr", "dac");
  long FtoD = findNumberOfPaths(namesToId, adj, order, "fft", "dac");
  long DtoF = findNumberOfPaths(namesToId, adj, order, "dac", "fft");
  long FtoO = findNumberOfPaths(namesToId, adj, order, "fft", "out");
  long DtoO = findNumberOfPaths(namesToId, adj, order, "dac", "out");
  ...
  cout << "Answer: " <<  StoF * FtoD * DtoO << endl;
  cout << "Answer: " <<  StoD * DtoF * FtoO << endl;
