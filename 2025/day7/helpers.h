...
vector<string> splitString(const string& s, char delimiter, bool removeEmpty = true) {
    vector<string> result;
    string current;

    for (char c : s) {
        if (c == delimiter) {
            if ((removeEmpty && !current.empty()) || (!removeEmpty)) result.push_back(current);
            current.clear();
        } else {
            current += c;
        }
    ...
    if ((removeEmpty && !current.empty()) || (!removeEmpty)) result.push_back(current); // last token
    return result;
}
